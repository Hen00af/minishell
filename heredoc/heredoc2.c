...
/*   Updated: 2025/06/25 21:50:40 by nando            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*run_heredoc(const char *delimiter, bool need_expand)
{
	char	*path;
	int		fd;
	char	*line;
...
void	process_heredoc(t_command *cmd)
{
	...
	while (node)
	{
		...
		{
			tmpfile = run_heredoc(redir->filename, redir->need_expand);
			free(redir->filename);
